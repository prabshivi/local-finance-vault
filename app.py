...
import re
import pandas as pd
import streamlit as pd_st  # We use standard streamlit package
import streamlit as st
import plotly.express as px
import plotly.graph_objects as go

from core.crypto import generate_transaction_hash, suggest_regex_pattern
from core.db import (
...
from core.rules import clean_merchant_name, apply_rules_to_staged
...
# Custom css for modern dark-mode & glassmorphism
st.markdown("""
<style>
    ...
    /* Sleek gradient background for structural cards */
    .metric-card {
    ...
    /* Decryption lock screen wrapper */
    .lock-container {
        max-width: 480px;
        margin: 100px auto;
        padding: 40px;
...
if not st.session_state.decrypted:
    ...
    st.markdown("<h1 style='color: #ffffff; margin-bottom: 10px;'>Vault Finance 🔒</h1>", unsafe_allow_html=True)
    st.markdown("<p style='color: #9CA3AF; margin-bottom: 30px;'>Enter your Master Passphrase to decrypt your offline personal finance vault.</p>", unsafe_allow_html=True)
    ...
    with col2:
        decrypt_btn = st.button("Unlock Vault", use_container_width=True, type="primary")
...
all_accounts = get_accounts(conn)
...
with tab_overview:
    ...
    with m_col1:
        ...
        <div class="metric-card">
            ...
            <div class="metric-value {'value-positive' if net_worth >= 0 else 'value-negative'}">${net_worth:,.2f}</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with m_col4:
        ...
        <div class="metric-card">
            ...
            <div class="metric-value {'value-positive' if savings_rate >= 0 else 'value-negative'}">{savings_rate:.1f}%</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    if not df_nw.empty:
        ...
        fig_nw.update_layout(
            ...
            yaxis=dict(gridcolor='rgba(255,255,255,0.05)', title="Net Worth ($)"),
        ...
        if st.button("Save Ledger Changes"):
            ...
            for idx, changes in edits.items():
                if 'category_name' in changes:
                    ...
                    tx_id = int(df_display.iloc[idx]['id'])
                    # Find category ID
...
with tab_ingestion:
    ...
    with col_ing1:
        ...
        parse_btn = st.button("Parse Statement File", use_container_width=True, type="primary")
    ...
    if st.session_state.staged_transactions is not None and len(st.session_state.staged_transactions) > 0:
        ...
        with col_commit1:
            if st.button("Commit Staged Rows to Vault Database", use_container_width=True, type="primary"):
        ...
        with col_commit2:
            if st.button("Discard Staged Statement", use_container_width=True):
                st.session_state.staged_transactions = None
                st.session_state.rule_suggestions = []
                st.rerun()
    ...
    if not mortgage_accs:
        ...
        if m_details is None:
            ...
            fig_curve = px.line(
                ...
                color_discrete_map={
                    'Standard Payment': '#EF4444', # Rose
                    'Accelerated Payment': '#10B981' # Emerald
                }
            )
            fig_curve.update_layout(
                ...
                plot_bgcolor='rgba(0,0,0,0)',
                font_color='#ffffff',
                xaxis=dict(gridcolor='rgba(255,255,255,0.05)', title="Months from Now"),
                yaxis=dict(gridcolor='rgba(255,255,255,0.05)', title="Remaining Debt Balance ($)"),
                margin=dict(t=40, b=10, l=10, r=10)
            )
...
with tab_rules:
    ...
    acc_df = pd.DataFrame(all_accounts)
    if not acc_df.empty:
        st.dataframe(
            acc_df[['name', 'type', 'institution', 'balance', 'currency']],
            use_container_width=True
        )
    else:
        ...
        with st.form("new_account_form"):
            ...
            if submit_acc:
                if not new_acc_name:
                    ...
                    add_account(conn, new_acc_name, new_acc_type, new_acc_inst, new_acc_bal, new_acc_cur)
    ...
    with st.expander("Remove Account"):
        ...
        if st.button("Delete Selected Account"):
            target_del = next(a for a in all_accounts if a['name'] == acc_to_delete)
    ...
    with col_c1:
        if st.button("Update Budget Changes"):
            ...
            for idx, changes in c_edits.items():
                cat_id = int(df_cats.iloc[idx]['id'])
                orig_cat = df_cats.iloc[idx]
